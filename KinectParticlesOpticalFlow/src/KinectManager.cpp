...
{
	opticalFlowScratchMat.setTo( 0 );
	colorPixelsMat.copyTo( opticalFlowScratchMat, depthPixelsCharMat );
...
void KinectManager::debugDraw2D()
{
	...
	if( imagesNeedUpdating )
	{
		lock();
			...
			maskedColorImage.setFromPixels( opticalFlowScratch );
	...
	if( colorImage.isAllocated() )
	{
		lock();
			...
			ofPushMatrix();
				...
				ofSetColor( ofColor::red );
				ofCircle( testDepthMapPos + ofVec2f(640 + (10 * 2), 10), 6 );

		//cout << testDepthMapPos << endl;
		
				ofSetColor( ofColor::white );
				if( doOpticalFlow )
				{
					maskedColorImage.draw( (640 * 2) + (10 * 2), 10, 640, 480);
...
// --------------------------------------------------------------------------------
//
ofxKinect* KinectManager::getKinectDevice()
{
	return &kinect;
}
